...
const STATUS_VARIANTS = {
    ...
    delayed: 'bg-danger/10 border-danger/40 text-danger glow-danger',
...
const BottomTimeline = memo(function BottomTimeline() {
    ...
    return (
        <div className="h-16 glass-strong border-t border-border flex items-center px-4 gap-3 shrink-0 overflow-x-auto scroller-hide">
            <span className="text-[10px] text-text-muted uppercase tracking-wider whitespace-nowrap font-bold mr-2 flex items-center gap-1.5">
                <Clock size={12} />
                Timeline
            </span>
            {recentTasks.map((t) => (
                <button
                    ...
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-full border hover:-translate-y-0.5 hover:shadow-lg transition-all cursor-pointer shrink-0 ${STATUS_VARIANTS[t.status] || STATUS_VARIANTS.idle}`}
                >
